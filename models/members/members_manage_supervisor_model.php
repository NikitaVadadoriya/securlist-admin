...
class members_manage_supervisor_Model extends Model
{
    ...
    public function get_all_supervisor()
    {
        ...
        parent::__construct();
        $data = $this->db->prepare("call get_area_by_manager(?)");
    ...
    public function add_supervisor()
    {
        ...
        if (isset($_POST["user_fname"]) && isset($_POST["user_lname"]) && isset($_POST["user_mail"]) && isset($_POST["user_phone"])) {
            ...
            if (count($check_user) >0) {
                ...
                header("location: " . URL . $session_user_type . "/manage_supervisor");
                exit();
            } else {
                $user_type = 3;
                $user_add = "";
                ...
                if ($result[0]["user_id"] > 0) {
                    ...
                    if ($_POST["user_area"] > 0) {
                        ...
                        if (!$is_inserted) {
                            ...
                            Session::set('danger', "User Zone not set up ! Update Zone Setting By going to Update User ");
                            header("location: " . URL . "members/manage_supervisor");
                            exit();
                        } else {
    ...
    public function get_supervisor_detail($user_mail)
    {
        ...
        $data->execute();
        $result["supervisor_data"] = $data->fetchAll();
