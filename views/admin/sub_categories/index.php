        ...
        <div class="card xl:col-span-2 mb-4">
            <div class="card-body flex flex-col p-6">
                <header class="flex mb-5 items-center border-b border-slate-100 dark:border-slate-700 pb-5 -mx-6 px-6">
                    <div class="flex-1">
                        <div class="card-title text-slate-900 dark:text-white">Add New Sub Category</div>
                    </div>
                </header>
                <div class="card-text h-full ">
                    <form class="space-y-4" method="post" id="add_sub_category_form"
                        action="<?=URL . admin_link?>/sub_categories/add_sub_category">
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-7">
                            ...
                            <div class="input-area relative">
                                <label for="credits" class="form-label">Require Credits (1 Credit = 1$)</label>
                                ...
                                <table
                                    ...
                                    <thead class=" bg-slate-200 dark:bg-slate-700">
                                        <tr>
                                            ...
                                            <th scope="col" class="table-th">Sub Categories</th>
                                            <th scope="col" class="table-th">Credits (1 Credit = 1$)</th>
                                            <th scope="col" class="table-th">Action</th>
                                        </tr>
                                    </thead>
                                    <tbody
                                        class="bg-white divide-y divide-slate-100 dark:bg-slate-800 dark:divide-slate-700">
                                        <?php foreach ($sub_categories as $index => $data) { ?>
                                        <tr data-no.="<?= $index + 1 ?>"
                                            data-sub-cat-id="<?= $this->encrypt_string($data['id']) ?>"
                                            data-credits="<?= $data['listing_price'] ?>"
                                            ...
                                            <td class="table-td ">
                                                <?php
                                                    if($data['isDeleted'] === 1) {
                                                        ?>
                                                <button type="button"
                                                    onclick="handleBtnClick('<?= $this->encrypt_string($data['name']) ?>', 1)">
                                                    Activate
                                                </button>
                                                <?php
                                                    } else {
                                                ...
                                                <div class="flex space-x-3 rtl:space-x-reverse">
                                                    ...
                                                    <button class="action-btn" type="button"
                                                        onclick="handleBtnClick('<?= $this->encrypt_string($data['name']) ?>', 2)">
...
<div class="modal fade fixed top-0 left-0 hidden w-full h-full outline-none overflow-x-hidden overflow-y-auto"
    id="sub_cat_modal" tabindex="-1" aria-labelledby="sub_cat_modal" aria-hidden="true">
    <!-- BEGIN: Modal -->
    <div class="modal-dialog relative w-auto pointer-events-none">
        <div
            ...
            <div class="relative bg-white rounded-lg shadow dark:bg-slate-700">
                ...
                <!-- Modal body -->
                <form method="post" action="<?=URL . admin_link?>/sub_categories/change_sub_cat_status">
                    <div class="p-6 space-y-4">
                        <div class="form-group">
                            <input type="hidden" name="sub_cat_name" id="sub_cat_name">
    ...
    <div class="modal-dialog relative w-auto pointer-events-none">
        <div
            ...
            <div class="relative bg-white rounded-lg shadow dark:bg-slate-700">
                ...
                <form id="EditSubCategoryData">
                    <div class="p-6 space-y-6">
                        <div class="grid grid-cols-1 gap-7">
                            <div class="input-area relative">
                                <label for="EditsubCategoryName" class="form-label">Sub Categories Name</label>
                                <input type="text" class="form-control" name="EditsubCategoryName"
                                    id="EditsubCategoryName" placeholder="Enter Sub Categories Name">
                            </div>
                            <div class="input-area relative">
